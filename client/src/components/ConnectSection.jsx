...
function LeetCodeCard({ config, onUpdate }) {
  ...
  async function handleConnect(e) {
    ...
    setSuccess(null);
    try {
      const res = await fetch(apiUrl("/api/auth/leetcode"), {
        method: "POST",
  ...
  async function handleDisconnect() {
    setLoading(true);
    try {
      await fetch(apiUrl("/api/auth/leetcode"), { method: "DELETE" });
      setSuccess(null);
      setError(null);
      onUpdate();
    } finally {
        ...
        <form onSubmit={handleConnect}>
          ...
          >
            ...
            <div>
              ...
              <br />
              2. Open DevTools (<code>F12</code>) → <strong>Application</strong>{" "}
              → <strong>Cookies</strong> → <code>https://leetcode.com</code>
              <br />
              3. Copy the <code>LEETCODE_SESSION</code> cookie value and paste
              it below.
            </div>
          </div>
...
function GitHubOAuthSetup({ onConfigured }) {
  ...
  async function handleSave(e) {
    ...
    setError(null);
    try {
      const res = await fetch(apiUrl("/api/auth/github/oauth-setup"), {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
  ...
  return (
    <div>
      ...
      <div style={{ marginBottom: 16 }}>
        ...
        >
          ...
          <div>
            ...
            <code
              style={{ fontFamily: "monospace", color: "var(--emerald-light)" }}
            >
              https://code-sync-3sld.onrender.com
            </code>
          </div>
          <div>
            <span style={{ color: "var(--violet-light)", fontWeight: 600 }}>
          ...
          <div>
            ...
            Paste the <strong>Client ID</strong> and generate/paste a{" "}
      ...
      <form onSubmit={handleSave}>
        ...
        >
          {saving ? <span className="spinner" /> : "💾"} Save &amp; Enable
          Sign-in
        </button>
      </form>
    </div>
...
function GitHubCard({ config, onUpdate }) {
  ...
  // Load OAuth status
  useEffect(() => {
    fetch(apiUrl("/api/auth/github/oauth-status"))
      .then((r) => r.json())
      .then(setOauthStatus)
  ...
  async function handleDisconnect() {
    setLoading(true);
    try {
      await fetch(apiUrl("/api/auth/github"), { method: "DELETE" });
      onUpdate();
    } finally {
      setLoading(false);
    }
  ...
  async function handlePATConnect(e) {
    ...
    try {
      ...
      if (data.valid) {
        ...
        setPatError(data.error || "Invalid token");
      }
    } catch {
      ...
      {isConnected ? (
        ...
        <>
          ...
          <div className="alert success mb-16">
            <span className="alert-icon">✅</span>
            <span>
              Signed in with GitHub. Repositories will be listed from this
              account.
            </span>
          </div>
          <button
          ...
          {usePat ? (
            ...
            <form onSubmit={handlePATConnect}>
              ...
              >
                ...
                <div>
                  ...
                  <br />
                  Generate a Personal Access Token with <code>repo</code> scope
                  to allow CodeSync to push to your repository.
                  <br />
                  <a
              ...
              >
                {loading ? <span className="spinner" /> : "🔑"} Connect with
                Token
              </button>
            </form>
            ...
            <>
              <div className="alert info mb-16">
                <span className="alert-icon">✅</span>
                <span>
                  OAuth App configured. Click below to sign in with GitHub.
                </span>
              </div>
              <a
  ...
  return (
    <div className="alert info" style={{ marginTop: 24 }}>
      ...
      <div>
        ...
        <code style={{ fontFamily: "monospace", color: "var(--violet-light)" }}>
          /extension
        </code>{" "}
        folder as an unpacked extension to auto-capture your LeetCode cookie —
        no DevTools needed.
      </div>
    </div>
  );
    ...
    <div className="page">
      <div className="page-header">
        ...
        <p className="page-subtitle">
          Link your LeetCode session and GitHub Personal Access Token to enable
          syncing.
        </p>
      </div>
      ...
      {bothConnected && (
        <div className="alert success mb-24">
          <span className="alert-icon">✅</span>
          <span>
            Both accounts connected! Head to <strong>Repository</strong> to pick
            where to sync.
          </span>
        </div>
      )}
