...
function classifyLine(msg) {
  ...
  if (
    ...
    msg.includes("Push complete")
  )
    return "success";
  if (msg.toLowerCase().includes("error") || msg.includes("FATAL"))
    return "error";
  return "default";
}
...
export default function SyncSection({ config, isReady }) {
  ...
  function startSSE(id) {
    ...
    const es = new EventSource(apiUrl(`/api/sync/${id}/stream`));
    esRef.current = es;
  ...
  async function handleStartSync() {
    ...
    try {
      const res = await fetch(apiUrl("/api/sync/start"), {
        method: "POST",
  ...
  const repoShort =
    config?.targetRepoName?.split("/").pop() || config?.targetRepoUrl || "—";
    ...
    <div className="page">
      <div className="page-header">
        ...
        <p className="page-subtitle">
          Fetch all your LeetCode accepted submissions and commit them with
          original timestamps.
        </p>
      </div>
      ...
      {!isReady && (
        <div className="alert warn mb-24">
          ...
          <span>
            Complete all steps in <strong>Connect</strong> and{" "}
            <strong>Repository</strong> before syncing.
          </span>
        </div>
      )}
      ...
      {(syncing || lines.length > 0) && (
        <Terminal lines={lines} isRunning={syncing} jobStatus={jobStatus} />
      )}
      ...
      {jobStatus === "success" && (
        <div className="alert success mt-16">
          ...
          <span>
            {dryRun
              ? "Dry run complete!"
              : "Sync complete! Check your GitHub repo for new commits."}{" "}
